...
//if文ではempty関数で$limitに値が入っているかを確認し、もし値がセットされていたらLIMIT句をSQLに指定します。LIMIT句は名前の通り、データを取得する件数の最大数を指定することができます。
    if( !empty($limit) ) {
			$sql = "SELECT * FROM message ORDER BY post_date ASC LIMIT $limit";
		} else {
			$sql = "SELECT * FROM message ORDER BY post_date ASC";
		}
//ORDER BY句に「ASC」を指定して昇順（登録された順）で取得、「DESC」で降順
		// $sql = "SELECT * FROM message ORDER BY post_date ASC";
...
return;//「download.php」はページを表示しないため明示的にreturn;を記述しています。
?>